...
app = typer.Typer(help="Extract structured content from voice memo transcriptions into Obsidian daily notes.")
...
def collect_files(file_arg: Optional[str], input_dir_arg: Optional[str]) -> list[Path]:
    if file_arg:
    ...
    input_dir = resolve_input_dir(input_dir_arg)
...
def run(
    ...
    input_dir: Annotated[Optional[str], typer.Option("--input-dir", "-i", help="Directory containing transcriptions.")] = None,
    file: Annotated[Optional[str], typer.Option("--file", "-f", help="Process a single file.")] = None,
    vault_path: Annotated[Optional[str], typer.Option("--vault-path", "-v", help="Path to Obsidian vault.")] = None,
    note_dir: Annotated[str, typer.Option("--note-dir", "-d", help="Subdirectory for daily notes.")] = DEFAULT_NOTE_DIR,
    note_date_str: Annotated[Optional[str], typer.Option("--date", help="Override the date (YYYY-MM-DD).")] = None,
    verbose: Annotated[bool, verbose_option()] = False,
    debug: Annotated[bool, debug_option()] = False,
):
    """Process voice memo transcriptions into Obsidian daily notes."""
    ...
    if note_date_str:
        try:
            ...
            typer.echo(f"Error: invalid date format '{note_date_str}'. Use YYYY-MM-DD.", err=True)
            raise typer.Exit(1)
    ...
    # Build provider
    try:
        provider_obj = resolve_provider(PROVIDERS, provider, model, debug=debug, verbose=verbose)
    except Exception as e:
    ...
    for f in files:
        result = process_file(f, provider_obj, verbose)
    ...
    if dry_run:
        for d, group in groupby(results, key=lambda x: x[1]):
            ...
            for _, _, res in group:
                combined.thoughts.extend(res.thoughts)
                combined.actions.extend(res.actions)
                combined.gratitude.extend(res.gratitude)
